...
// import AWS from 'https://sdk.amazonaws.com/js/aws-sdk-2.1595.0.min.js'
...
class TalkingHead {
  ...
  */
  ...
  accumulate(input) {
    let output = [0]; // Initialize the output array with 0 at the beginning
    let sum = 0; // Initialize the sum variable

    for (let i = 0; i < input.length - 1; i++) {
      sum += input[i]; // Add the current value to the sum
      output.push(sum); // Push the sum to the output array
    }
  ...
  convertto_lipsyncWordsToVisemes(word, visArr) {
    var o = { words: word.toUpperCase(), visemes: [], times: [], durations: [], i: word.length };
    // console.log("Word is : " + word)
    //console.log(visArr)

    visArr.forEach(item => {
      o.visemes.push(item.value);
      o.times.push(item.time / 1000);
      o.durations.push(item.duration);
    });

    o.times = this.accumulate(o.durations)

    // o.visemes = this.pollyToOculusMap(o.visemes)

    return o;
  }

  setAudioStream(aStream) {
    this.aStreamData = aStream
  }
    ...
    if (this.speechQueue.length) {
      ...
      if (line.emoji) {
        ...
        try {
          ...
          const audio = this.aStreamData.audio[this.aStreamDataCounter];
          ...
          if (audio) {
            ...
            const data1 = { "timepoints": this.aStreamData.timepoints[this.aStreamDataCounter] }
            ...
            this.aStreamDataCounter = this.aStreamDataCounter + 1;

            // Add to the playlist
            this.audioPlaylist.push({ anim: line.anim, audio: audio });
            // console.log("audiPlaylist: ")
            // console.log(JSON.stringify(this.audioPlaylist, null, 4));
            this.onSubtitles = line.onSubtitles || null;
            this.resetLips();
            if (line.mood) this.setMood(line.mood);
            this.playAudio();
